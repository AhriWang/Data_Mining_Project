...
class DataLoader:
    ...
    def load_json(self, file_path: str) -> Tuple[List[str], List[int]]:
        ...
        try:
            ...
            # 检查是否有NaN值
            for item in data:
                if 'label' not in item or 'text' not in item:
                    raise ValueError("JSON文件中缺少标签或文本字段!")
                if pd.isna(item['label']) or pd.isna(item['text']):
                    raise ValueError("JSON文件中存在NaN值!")
    ...
    def load_and_split_data(self,
        ...
        List[str], List[str], List[str], List[int], List[int], List[int]]:
        """
        加载并分割数据为训练集、验证集和测试集
...
def load_data_from_dir(data_dir: str, file_type: str = 'csv') -> Tuple[List[str], List[int]]:
    ...
    for file_name in os.listdir(data_dir):
        if file_name.endswith(file_type):
            ...
            if file_type == 'csv':
                file_texts, file_labels = loader.load_csv(file_path)
            elif file_type == 'json':
                file_texts, file_labels = loader.load_json(file_path)
